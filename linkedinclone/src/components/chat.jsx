...
export default function ChatPage() {
  ...
  const pictureUrl = decodeURIComponent(picture)
  const loginUser = useGetLoginUser()
  const [roomid, setroomid] = useState()
  const [online, setonline] = useState(false)
  const online_users = useSelector(state=>state?.user?.online_users)

  useEffect(() => {
    if (user && loginUser?._id) {
      let roomIdInstant = createRoom(loginUser?._id, user);
      setroomid(roomIdInstant)
      socket.emit('join-chat', roomIdInstant)
  ...
  return (
    <div className="flex flex-col h-[90vh] bg-white shadow-lg rounded-xl overflow-hidden md:max-w-4/6 lg:max-w-3/6 mx-auto">
      ...
      <div className="p-4 border-b flex items-center gap-3 bg-black text-white shadow-sm">
        <img src={pictureUrl} alt="" className="w-10 h-10 rounded-full shadow" />
        <div className="">
          <h2 className="text-lg font-semibold">{name}</h2>
          <p className="text-sm opacity-80">
            {online ? (
              <span className="flex items-center gap-2 p-0 m-0">
                Active <span className="w-3 h-3 bg-green-500 rounded-full inline-block"></span>
              </span>
            ) : (
             <span className="flex items-center gap-2 p-0 m-0">
                Offline 
